...
--Faça uma view que mostre a quantidade de compras feitas por cada cliente

create view v_ex1 as 

select cliente.chave_cliente, cliente.nome, 
count(*) as QUANTIDADE
from cliente join compra 
on cliente.chave_cliente = compra.cliente
group by cliente.chave_cliente,cliente.nome

--Faça uma view que retorne os 3 produtos mais caros, ordenados do mais caro para o mais barato

create view v_ex2 as 
select top 3 * from produto 
order by preco desc

select descricao from v_ex2

--Faça uma função que dado um id de compra, 
--recebido por parâmetro, retorne o nome 
--dos produtos e a quantidade que foram comprados
...
on produto.chave_produto = itens_compra.produto
 ...
 select * from f_compras(200) 

-- Faça uma função que dado um id de compra, 
--recebido por parâmetro,  retorne a quantidade 
--de produtos que foram comprados
select sum(quantidade) from itens_compra where compra = 1

 create function f_qtd_comprado(@id_compra int)
 returns int
 as 
 begin
 declare @qtd int
 ...
 select compra.codigo_nota, dbo.f_qtd_comprado(1) 
 from compra where chave_compra = 1


 --crie um novo atributo na tabela cliente 
 -- RG Varchar(20) 

 alter table cliente add RG varchar(20) 


 --faça um procedimento que mostre o nome e o preço total 
 --dos produtos comprados em cada compra, 
 --bem como o valor total da compra. O layout deve ficar
